...
MODELS_TO_BENCHMARK = [
    ...
    {"model_id": "mrm8488/t5-small-finetuned-for-xquad-tydiqa-mlqa", "size_category": "small"},
...
def qa_benchmark_worker(chunks_subset, gpu_id, model_id):
    """Fonction exécutée par chaque processus pour la génération de QA."""
    ...
    try:
        ...
        print(f"Erreur lors du chargement du modèle {model_id} sur GPU:{gpu_id}: {e}")
        return [], 0
    ...
    for chunk in chunks_subset:
        ...
        try:
            ...
            generated = qa_pipeline(input_text, max_length=128, num_return_sequences=1)
            generated_text = generated[0]['generated_text']
            # Gère les cas où <sep> n'est pas généré
            question, answer = (generated_text.split('<sep>', 1) + ["N/A"])[:2]
            results.append({"chunk": chunk, "question": question.strip(), "answer": answer.strip()})
        except Exception as e:
            print(f"Erreur de génération sur GPU:{gpu_id} avec le chunk : {chunk[:50]}... Erreur: {e}")
            results.append({"chunk": chunk, "question": "ERREUR", "answer": "ERREUR"})
        generation_times.append(time.time() - start_time)

    del qa_pipeline
    torch.cuda.empty_cache()
    return results, sum(generation_times)
...
def main():
    """Orchestre le prétraitement complet PUIS le benchmark des modèles de QA."""

    # --- 1. PIPELINE DE PRÉTRAITEMENT COMPLET ---
    checkpoint_file = "checkpoint__df_chunk_ready.parquet"
    if os.path.exists(checkpoint_file):
        print(f"[INFO] Chargement des données déjà prétraitées depuis '{checkpoint_file}'...")
        df_chunk = pd.read_parquet(checkpoint_file)
    else:
        print("[INFO] Fichier de checkpoint non trouvé. Démarrage du pipeline de prétraitement.")
        languages, max_docs_per_lang = ['fr', 'en'], 10000  # Réduire pour un test rapide, augmenter pour la production
        ...
        print("[INFO] Découpage des textes...")
    ...
    print(f"\n[INFO] Prétraitement terminé. {len(df_chunk)} chunks prêts pour le benchmark QA.")

    # Utiliser un échantillon pour un benchmark plus rapide
    sample_size = 10000
    if len(df_chunk) > sample_size:
        df_chunk_sample = df_chunk.sample(n=sample_size, random_state=42)
    else:
        df_chunk_sample = df_chunk
    ...
    # --- 2. BENCHMARK DES MODÈLES DE GÉNÉRATION QA ---
    if not torch.cuda.is_available():
        print("Aucun GPU CUDA n'est disponible. Le script s'arrête.")
        return
    ...
    for model_info in MODELS_TO_BENCHMARK:
        ...
        pool_args = [(chunks_per_gpu[i].tolist(), i, model_id) for i in range(num_gpus)]

        total_gen_time, all_generated_qa = 0, []
        with mp.Pool(processes=num_gpus, context=mp.get_context('spawn')) as pool:
            worker_outputs = pool.starmap(qa_benchmark_worker, pool_args)
            for qa_list, time_taken in worker_outputs:
                all_generated_qa.extend(qa_list)
                total_gen_time += time_taken

        if not all_generated_qa:
            print(f"Aucun résultat généré pour {model_id}. Passage au modèle suivant.")
            continue

        generated_df = pd.DataFrame(all_generated_qa)
        predictions = (generated_df["question"] + " " + generated_df["answer"]).tolist()
        ...
        try:
            ...
            print(f"Erreur lors du calcul du BERTScore : {e}")
            avg_bert_f1 = -1
        ...
        try:
            ...
            print(f"Erreur lors du calcul du ROUGE score : {e}")
            rouge_l_score = -1
        ...
        print(f"Résultats pour {model_id}: {results}")
